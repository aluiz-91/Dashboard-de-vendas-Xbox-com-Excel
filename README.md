🎮 Xbox Game Pass Subscriptions Sales Dashboard

📋 Sobre o Projeto

Este projeto consiste em um Dashboard Executivo de Vendas e Assinaturas do Xbox Game Pass, desenvolvido como parte de um Desafio de Projeto na formação/bootcamp de Excel com IA da DIO (Digital Innovation One) em parceria com a Santander Open Academy.

O objetivo do projeto é consolidar dados de vendas de assinaturas de pacotes e passes de temporada do ecossistema Xbox (como EA Play Pass e Minecraft Season Pass), utilizando Microsoft Excel integrado a ferramentas de Inteligência Artificial Generativa para otimizar cálculos, organização visual e geração de insights de negócios.

🖼️ Prévia do Dashboard

🎯 Funcionalidades e Destaques do Painel

Métricas Principais (Cards de KPI):

Total Subscriptions EA Season Pass: Acompanhamento do faturamento do passe EA Play.

Total Subscriptions Minecraft Season Pass: Monitoramento de receitas geradas por pacotes de temporada do Minecraft.

Total Subscriptions Xbox Game Pass: Comparativo dinâmico de assinaturas acumuladas por modalidade.

Filtros e Interatividade:

Segmentador de Período (Subscription Type): Alternância e filtragem dinâmica entre tipos de assinaturas (Annual, Monthly, Quarterly).

Controle de Apuração: Exibição clara de datas de apuração e datas de atualização dos dados.

Design e UI/UX no Excel:

Layout tematizado com a identidade visual da marca Xbox.

Interface limpa, intuitiva e sem poluição visual, otimizada para apresentações executivas.

🛠️ Tecnologias e Ferramentas Utilizadas

Microsoft Excel:

Fórmulas e Funções de Agregação

Tabelas Dinâmicas (Pivot Tables) e Gráficos Dinâmicos

Segmentadores de Dados (Slicers)

Formatação Condicional e Design Dashboard

Inteligência Artificial (IA Generativa - Copilot / ChatGPT):

Suporte na criação de fórmulas avançadas.

Otimização da estrutura e auxílio no planejamento do layout do dashboard.

📂 Estrutura do Repositório

├── README.md             # Documentação técnica do projeto
└── xbox game pass.xlsx   # Planilha Excel com os dados e o Dashboard interativo


🚀 Como Visualizar e Executar

Faça o download do arquivo xbox game pass.xlsx disponível neste repositório.

Abra o arquivo no Microsoft Excel (versão Desktop recomendada).

Utilize o painel de Subscription Type na lateral esquerda para filtrar interativamente os dados entre os períodos Annual, Monthly e Quarterly.

👨‍💻 Autor

Desenvolvido por André Luiz durante o bootcamp de Excel com IA da DIO.
